...
from torch.nn import Module, ModuleList, Linear, LayerNorm, Parameter


class BilinearDecoder(Module):

    def __init__(self, emb_dim, bias=False, act=None) -> None:
        super(BilinearDecoder, self).__init__()
        self.weight = Parameter(torch.FloatTensor(emb_dim, emb_dim))
        self.act = act
        if bias:
            self.bias = Parameter(torch.FloatTensor(emb_dim))
        else:
    ...
    def forward(self, hd, hm):
        tmp = torch.mm(self.weight, hm)
        tmp = torch.mm(hd, tmp)
        if self.act:
            return self.act(tmp.reshape(-1))
        else:
            return tmp.reshape(-1)

    def __repr__(self):
        return f'{self.__class__.__name__} ' \
               f'Hd {self.weight.shape} Hm   ' \
               f'bias:{self.bias is not None}  ' \
               f'act:{self.act.__name__ if self.act is not None else None} '


class OGNNMDA(Module):
    def __init__(self, params):
        super(OGNNMDA, self).__init__()
        self.params = params
        self.linear_trans_in = ModuleList()
        # self.linear_trans_out = Linear(params['hidden_channel'], params['out_channel']) # Decoder
        self.decode = BilinearDecoder(params['hidden_channel'])
        ...
        self.params_others = list(self.linear_trans_in.parameters()) + list(self.decode.parameters())
    ...
    def forward(self, x, edge_index):
        ...
        res = self.decode(x[:self.params['drugs'], :], x[self.params['drugs']:, :].T)
